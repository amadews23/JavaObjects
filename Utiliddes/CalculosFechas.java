...
import Gestion.EnumTemporadaPrecio;
import Gestion.EnumClientePrecio;
...
public class CalculosFechas {
    ...
    public static EnumTemporadaPrecio devolverTemporada(GregorianCalendar fechaEntrada) {
        ...
        if ( (fechaEntrada.get(Calendar.DAY_OF_YEAR) >= diaAnyoNuevo) && (fechaEntrada.get(Calendar.DAY_OF_YEAR) <= diaFinNavidades) ) {
     ...
     * @return
     */
    public static EnumClientePrecio devolverTipoCliente(Persona persona) {

        EnumClientePrecio tipoCliente = null;
        int edad = persona.getEdad();
        
        if (edad >= EnumClientePrecio.BEBE.getEdadMin() && edad <= EnumClientePrecio.BEBE.getEdadMax() ) {
            return EnumClientePrecio.BEBE;
        }
        if ( edad <= EnumClientePrecio.NINYO.getEdadMax() ) {
            return EnumClientePrecio.NINYO;
        }
        if ( (edad >= EnumClientePrecio.ADULTO.getEdadMin()) && ( edad <= EnumClientePrecio.ADULTO.getEdadMax() ) ) {
            return EnumClientePrecio.ADULTO;
        }
        if ( edad >= EnumClientePrecio.SENIOR.getEdadMin()) {
            return EnumClientePrecio.SENIOR;
        } 
        return tipoCliente;
    }
